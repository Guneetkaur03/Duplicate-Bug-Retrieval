...
def read_bug_dataset():
  """
    This function is used to read the bugs dataset

    Returns:
        (list, set): list of bug pairs and unique bug ids
  """
  
  bug_pairs = []
  bug_ids = set()

  #reading the pairs dataset
  with open(os.path.join(data, 'pairs.json'), 'r') as f:
    count = 0

    #considering only 1lakh data
    for line in f:
      count += 1
      if count > 10000:
        break
  ...
  #appending data of pairs to bug pairs txt file
  with open(os.path.join(data, 'bug_pairs.txt'), 'w') as f:
  ...
  #appending unique ids to bug_ids txt file
  with open(os.path.join(data, 'bug_ids.txt'), 'w') as f:
...
def textpreprocess(text):
  """
    This function is used to preprocess text

    Args:
        text (str): bug description text
...
def normalize_text(text):
  ...
  try:
    ...
    text = ' '.join([textpreprocess(token) for token in tokens])
...
def save_dict(set, filename):
  """
    Used to save dictionaries

    Args:
        set (dict): information being saved
...
def normalized_data(bug_ids):
  ...
  bug_statuses = set()
  text = []
  normalized_bugs = open(os.path.join(data, 'normalized_bugs.json'), 'w')
  with open(os.path.join(data, 'bugs.json'), 'r') as f:
    count = 0
    loop = tqdm(f)
    for line in loop:
      bug = json.loads(line)
...
def train_test_split(bug_pairs):
  random.shuffle(bug_pairs)
  split_idx = int(len(bug_pairs) * split)
...
def build_vocabulary(train_text):
  ...
  word_vocab = save_vocab(word_freq, n_words, 'word_vocab.pkl')
  print('character vocabulary')
  char_vocab = save_vocab(char_freq, n_chars, 'char_vocab.pkl')
  return word_vocab, char_vocab


def dump_bugs(word_vocab, char_vocab):
  bug_dir = os.path.join(data, 'bugs')
  ...
  with open(os.path.join(data, 'normalized_bugs.json'), 'r') as f:
    loop = tqdm(f)
    for line in loop:
